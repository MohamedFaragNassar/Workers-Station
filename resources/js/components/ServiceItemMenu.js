...
const ServiceItemMenu = ({node,id,service,close,openEdit}) => {
    ...
    return <>
        <div ref={node} id="simenu" className="w-40 shadow-md rounded-sm flex flex-col items-center absolute top-2 right-2 bg-white px-2 z-10 pt-2 pb-2">
            <button onClick={()=>handleOpenEdit()} id="simenubtn" className="px-2 py-2 w-full flex items-center hover:bg-gray-200">
                <i className="fas fa-edit"></i>
                <span className="ml-2">Edit Service</span>
            </button>
            <button onClick={()=>handleDeleteService()} id="simenubtn" className="px-2 py-2 w-full flex items-center hover:bg-gray-200">
