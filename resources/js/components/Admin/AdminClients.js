...
const AdminClients = () => {
    ...
    const handleDelUser = ()=>{
        dispatch(deleteUser(user.id,"client"))
        setIsOpen(false)
    }
    ...
    return <>
             <div className="mb-8 text-lg font-semibold">All Clients</div>
        {loading?<Spinner /> : error ? <Status message={error.message} />:null}
        <div className="w-full flex flex-col items-center">
            <div className="w-full flex items-center justify-between ">
                <span className="text-left w-1/4">Name</span>
                <span className="text-left w-1/4">Email</span>
                <span  className="text-left w-1/3">Address</span>
            ...
            {users?.map(user => 
                <div className="w-full flex items-center justify-between border rounded-lg p-4 mt-3">
                    ...
                    <span className="text-left w-1/4 break-words">{`${user.email}`}</span>
                    <span className="text-left w-1/3 break-words">{`${user.address}`}</span>
